...
public class Server3 {
    ...
    private static void startServer() {
        try (ServerSocket serverSocket = new ServerSocket(SERVER_PORT)) {
            System.out.println("Server3 dinlemede: " + SERVER_PORT);

            while (true) {
                Socket socket = serverSocket.accept();
                System.out.println("Server3 baglanti alindi: " + socket.getInetAddress().getHostAddress());
    ...
    private static void handleClientMessage(Socket socket) {
        try {
            ...
            while ((message = reader.readLine()) != null) {
                System.out.println("Server3'ten gelen mesaj: " + message);
                ...
                // Diğer mesajlar
                    ...
                    sendMessage(socket, "Server3: Mesaj alindi!");
    ...
    private static void sendCapacityInfo(Socket socket) {
        ...
        int server3Status = 1000;  // Örnek kapasite durumu

        // Zaman damgası (timestamp)
        long timestamp = Instant.now().getEpochSecond();

        // JSON formatında kapasite mesajı
        String responseMessage = String.format("{\"demand\": \"CPCTY\", \"server3_status\": %d, \"timestamp\": %d}", server3Status, timestamp);
    ...
    private static void connectToServer(String host, int port) {
        while (true) {
            try {
                ...
                System.out.println("Server3 -> " + host + ":" + port + " baglantisi basarili");
                break;  // Baglanti saglandiyse donguden cik
            } catch (IOException e) {
                System.out.println("Server3 -> " + host + ":" + port + " baglanti bekleniyor...");
                try {
                    Thread.sleep(5000);
